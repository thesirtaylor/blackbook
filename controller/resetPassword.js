...
module.exports = {
  sendToken: async (req, res) => {
    ...
    if (_is_error)
      return res.status(HTTP_STATUS.BAD_REQUEST).json(ERR(`Properly formatted Email required!`));
    try {
          ...
          if (newToken) {
            ...
            if (sM) {
              return res
                ...
                .json(
                  SUCCESS(`Password reset mail has been sent successfully to ${options.email}`)
                );
            }
            logger.info(`${user._id}: ${user.username}: Mail sending Failed.`);
            return res.status(HTTP_STATUS.SERVICE_UNAVAILABLE).json(ERR(`Mail sending Failed.`));
          } else {
            logger.info(`${user._id}: ${user.username}: Creation of new token failed, try again.`);
            return res
              .status(HTTP_STATUS.CONFLICT)
              .json(ERR(`Creation of new token failed, try again.`));
          }
        }
      } else {
        logger.info(`${user._id}: ${user.username}:Email not registered`);
        ...
        if (user) {
          ...
          if (save) {
            ...
            if (sM) {
              return res
                .status(HTTP_STATUS.OK)
                .json(SUCCESS(`Password successfully changed, Sign in with New Password`));
            }
            logger.info(`${user._id}: ${user.username}: Mail sending Failed.`);
            return res.status(HTTP_STATUS.SERVICE_UNAVAILABLE).json(ERR(`Mail sending Failed.`));
          } else {
            logger.info(
              `${user._id}: ${user.username}: Password not successfully changed, try again.`
            );
            return res
              .status(HTTP_STATUS.NOT_MODIFIED)
              .json(ERR(`Password not successfully changed, try again.`));
          }
        } else {
          logger.info(`No account linked to the provided recovery parameters`);
          return res
            .status(HTTP_STATUS.NOT_FOUND)
            .json(ERR(`No account linked to the provided recovery parameters`));
        }
      } else {
        logger.info(`Token not recognised`);
