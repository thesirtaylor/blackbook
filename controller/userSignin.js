...
module.exports = {
  signin: async (req, res) => {
    ...
    try {
      ...
      if (!user) {return res.status(HTTP_STATUS.NOT_FOUND).json(ERR(`This sign in parameter is not associated with any account. Check, re-type, and try again.`))}
      else {
        let compare  = await bcrypt.compare(options.password, user.password);
        if (!compare){return res.status(HTTP_STATUS.NOT_FOUND).json(ERR(`Incorrect Password.`))}
        else{
          if(!user.isVerified){return res.status(HTTP_STATUS.NOT_FOUND).json(ERR(`Can't sign in until email has been Verified.`))}
          else{
            let sign = await SIGN_TOKEN({user: user._id});
            if(sign){return res.status(HTTP_STATUS.ACCEPTED).json(SUCCESS(sign))}
            else{ return res.status(HTTP_STATUS.NOT_ACCEPTABLE).json(ERR(`No token signed.`))}
          }
        }
      }
    } catch (error) {
