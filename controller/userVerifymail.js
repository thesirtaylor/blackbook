...
module.exports = {
  verify: async (req, res) => {
    ...
    try {
      ...
      if (token) {
        ...
        if (!user)
          return res.status(HTTP_STATUS.NOT_FOUND).json(ERR(`Token not linked to any Account`));
        else {
          if (user.isVerified === true)
            return res.status(HTTP_STATUS.CONFLICT).json(ERR(`User has been verified already`));
          else {
          ...
          if (save) {
            ...
            return res
              ...
              .json(SUCCESS(`Account Verified, proceed to Sign in.`));
          }
        }
      } else
        return res.status(HTTP_STATUS.NOT_FOUND).json(ERR(`Token not found, request another.`));
    } catch (error) {
      logger.error(`${error}`);
  ...
  resend: async (req, res) => {
    ...
    try {
      ...
      if (user.isVerified) {
        return res.status(HTTP_STATUS.NOT_IMPLEMENTED).json(ERR(`This account has been verified.`));
      } else {
        let token = await Token.findOne({ _userId: user._id });
        if (token) {
          logger.info(`The last token you requsted hasn't expired. Check your email for it.`);
          return res
            .status(HTTP_STATUS.FORBIDDEN)
            .json(ERR(`The last token you requsted hasn't expired. Check your email for it.`));
        }
        let recreate = await Token.create({
          _userId: user._id,
        ...
        if (recreate) {
          ...
          if (sM) {
            return res
              .status(HTTP_STATUS.OK)
              .json(SUCCESS("Verification mail has been sent successfully to " + options.email));
          }
